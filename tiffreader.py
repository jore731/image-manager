...
class NEFImage(TIFFImage):
    ...
    @property
    def image_file(self):
        if not hasattr (self, "_image_file"): self._image_file = open(f'{self.imagePath}', 'rb')
    ...
    def createJSON (self, outpath=""):
        with open(f'{outpath}{self.noExtName}.json', 'w') as outfile:
